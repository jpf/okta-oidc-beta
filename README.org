...
*** Validating an OIDC =id_token= from Okta
    OIDC =id_token=s are JWTs and validating a JWT is easy. Below is a
...
*** /login
    ...
    #+BEGIN_SRC python
      ...
      def test_login_with_password(self):
          ...
          self.assertIn(sessionToken, rv.headers['Location'])   #+END_SRC
    #+END_SRC
*** /logout
    #+NAME: route_logout
    #+BEGIN_SRC python
...
** tests/test_flask_app.py
   This is how we generate an =RSA PRIVATE KEY= to sign our JWTs.
   #+BEGIN_SRC sh :results code
     openssl genrsa 1024
   #+END_SRC

   Here is what a private key should look like:
   #+NAME: private_key
   #+RESULTS:
   #+BEGIN_SRC sh
   ...
   And this is how we generate an =RSA PUBLIC KEY=, which we use to
   validate JWTs signed with the private key.
   ...
   #+BEGIN_SRC sh :var private_key=private_key :results code
     echo "$private_key" | sed -e 's/^[ ]*//' | openssl rsa -pubout
   #+END_SRC

   Here is what a public key should look like:
   #+NAME: public_key
   #+RESULTS:
   #+BEGIN_SRC sh
   -----BEGIN PUBLIC KEY-----
   ...
   #+BEGIN_SRC python :tangle templates/test_flask_app.py :noweb yes :exports none :padline no
         ...
         <div class="auth-content">
           <div class="auth-content-inner">
             <div class="primary-auth">
               <form action="/" class="primary-auth-form o-form o-form-edit-mode" method="post">
